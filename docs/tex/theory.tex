...
The deployed certificate is clipped to $[-1,V_{\max}]$ with $V_{\max}=1$. Suppose an open set
$O\subseteq\calX_0$ on which it saturates at the ceiling, $\Vhat\equiv V_{\max}$. Then on $O$,
for \emph{every} gain $\alpha>0$, every box $\calU$, and every margin $\gamma_m\ge0$:
\begin{enumerate}[label=(\alph*)]
...
is $\alpha(V_{\max}+\gamma_m)>0$, so
$s_\emptyset\equiv\alpha(V_{\max}+\gamma_m)>0$ and $O\subseteq\mathcal E$. The row is
...
ceiling. Scope: exact, per fixed $o$, for the clipped deployed parameterization.
\end{proposition}
\begin{proof}
...
=\alpha(V_{\max}+\gamma_m)$이다. \cref{prop:sinfty-decomp}의 표시에 대입하면 결론이 나오며,
$\alpha>0$과 $V_{\max}>0$에서 부호는 $\alpha$, $\calU$, $\gamma_m$의 값과 무관하다. (c)
$s_\emptyset$은 $O$ 위 상수 $\alpha(V_{\max}+\gamma_m)$이므로 그 gradient가 소멸한다. (d) (a)가
...
대해 $\{\abs{\Vhat}\ge1-10^{-6}\}$와 deployed singular flag가 \emph{양방향으로 동일한 집합}이다 ---
...
실제로 doom이라면 $\Vhat=V_{\max}$가 옳을 수 있다. 확정되는 것은 그 상태에서 배포 row가 어떤
...
$\{\abs{\Vhat^-}=V_{\max}\}$ 위에서 $\sigma=\alpha V_{\max}>0$은 parameterization의 산물이며
plant와 무관하다. 이 집합에서 \eqref{eq:sigma-hazard}를 그대로 평가하면 clip artifact가
label로 승격된다 --- 측정상 active step의 $1.3$--$2.3\%$. 따라서 $\psi$는 clip 집합 밖에서만
$\sigma$를 받고, 그 위에서는 $\tilde h=h$로 둔다(또는 \cref{prop:ceiling}를 무효화하는
saturating value map이 선행한다).
...
\cref{prop:sigma-valid}(c)의 보수성 --- timeout/stuck 상승 압력 --- 이다.
\end{remark}

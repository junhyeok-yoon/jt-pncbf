...
- **`infeasibility`** — defined in `02_control` §4: a filtered step is infeasible iff the half-space–box intersection is **empty**, or the row is **singular and violated** ($\|L_g h\| < 5 \times 10^{-4}$ and $L_f h + \alpha\, h_{\text{eff}} > 0$, a $u$-independent test). A singular row that is satisfied is feasible — every $u$ satisfies it and the filter is simply inactive. Aggregation: mean over active filtered steps of the per-step flag, then mean over episodes; episodes with no active filtered steps contribute zero.
  ...
  **The two clauses are reported separately.** Empty intersection and singular-and-violated are different states with different causes, and singular-and-satisfied is neither — it is a state on which the row holds for every admissible input. Any report of the infeasibility term carries the clauses apart, each with its step mass, so that a term is never read as a single failure signature when it is a mixture. Which infeasibility convention a system is scored under is read from that system's own scoring path and is never assumed from another system's.
...
**Active window.** A step is **active** when it precedes the episode's physical termination — the first step at which collision, goal, or out-of-bounds fires. An episode resolved as timeout or stuck has every step active, since neither predicate terminates the rollout. Per-step diagnostics aggregate over this window and no other: the `infeasibility` of §1, the `saturation_rate` of `02_control` §4.1, and any quantity reported as a share "over active steps". Because the rollout runs to `eval.max_steps` in every case, the active window is a strict subset of the recorded steps whenever the episode terminates physically, and a step observed by an instrument attached to the control loop is not thereby active.
